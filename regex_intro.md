...
- ed是unix上一个编辑器的名字, 后来发展出了sed, vi, vim等工具, grep的语法也是这个流派
- perl是正则表达式的集大成者, python的re库也是按照perl的语法设计的
...
以及一个用来修饰`\w`的量词`*`, 量词`*`的存在保证了信号名长度最小为1
...
其中的pattern就是正则表达式replace是一个字符串, 用来替代搜索到的内容
